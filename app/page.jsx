...
export default function Home() {
  return (
    <main className="max-h-screen container">
        ...
        >
         <SwiperSlide>
            <div className="relative w-full h-full"> {/* ปรับขนาดตามต้องการ */}
              <Image
                src="/assets/img_main/banner-1.png"
                fill
                style={{ objectFit: 'cover' }} // หรือ 'contain' ตามที่ต้องการ
              />
            </div>
          </SwiperSlide>
          <SwiperSlide>
        ...
        <CalendarComponent/>
        <div className="bg-[rgba(255,102,0,0.7)] w-[350px] rounded-lg p-3 mt-2 h-fit max-lg:w-[50%] max-sm:w-[90%]">
        ...
        <div className="text-[#FF6600] flex justify-between items-center px-2">
          <h3 className="text-2xl max-[1214px]:text-lg">กิจกรรมที่คุณอาจสนใจ</h3>
          <Link className='flex items-center space-x-2 text-2xl max-[1214px]:text-lg' href="#">ดูทั้งหมด <span><svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="1.5" class="size-6" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/></svg></span></Link>
        </div>
          <div className="grid grid-cols-3 mt-3 max-xl:grid-cols-3 max-lg:grid-cols-3 gap-4 w-auto max-md:grid-cols-2 max-lg:w-full max-sm:place-items-center max-[510px]:grid-cols-1">
            <CartEvent/>
            <CartEvent/>
            <CartEvent/>
            <CartEvent/>
